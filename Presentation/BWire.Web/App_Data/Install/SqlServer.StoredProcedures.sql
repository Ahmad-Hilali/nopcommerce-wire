...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''BWireFullTextCatalog'')
		CREATE FULLTEXT CATALOG [BWireFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + @index_name +  '] ON [BWireFullTextCatalog] WITH CHANGE_TRACKING AUTO'
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''BWireFullTextCatalog'')
		DROP FULLTEXT CATALOG [BWireFullTextCatalog]
	')
END
GO
